...
let isFlushing = false  //是否在执行队列
let isFlushPending = false //是否在等待执行队列

const queue: SchedulerJob[] = [] //程序队列
let flushIndex = 0  //当前执行的程序对应队列的索引

const pendingPreFlushCbs: SchedulerCb[] = []      //等待执行的程序队列的前置回调队列
let activePreFlushCbs: SchedulerCb[] | null = null //已经在执行的程序队列的前置回调队列
let preFlushIndex = 0   //当前执行的程序队列的前置回调对应队列的索引

const pendingPostFlushCbs: SchedulerCb[] = []//等待执行的程序队列的后置回调队列
let activePostFlushCbs: SchedulerCb[] | null = null//已经在执行的程序队列的后置回调队列
let postFlushIndex = 0//已经在执行的程序队列的后置回调队列
...
/**向队列加入SchedulerJob,并执行queueFlush开始执行队列 */
...
/**把SchedulerJob从队列中移除(取消执行此SchedulerJob) */
...
/**向等待执行的程序队列的前置回调队列添加回调 */
export function queuePreFlushCb(cb: SchedulerCb) {
  queueCb(cb, activePreFlushCbs, pendingPreFlushCbs, preFlushIndex)
}

/**向等待执行的程序队列的后置回调队列添加回调 */
