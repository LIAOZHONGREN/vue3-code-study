...
type ExtractMixin<T> = { Mixin: MixinToOptionTypes<T> }[T extends ComponentOptionsMixin ? 'Mixin' : never]
...
/**代理组件内部实例的代理处理程序(作用是隐藏一些内部属性不让外部调用(只暴露用户可以使用的属性)) */
